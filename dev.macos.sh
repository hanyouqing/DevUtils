#!/usr/bin/env bash
#
# AWS Utilities for macOS - Common AWS CLI commands wrapped as shell functions
# Usage: curl -sSL https://raw.githubusercontent.com/hanyouqing/devops-utils/main/dev.macos.sh | source /dev/stdin
# Or: source <(curl -sSL https://raw.githubusercontent.com/hanyouqing/devops-utils/main/dev.macos.sh)
#
...
_check_macos() {
  if ! _detect_macos; then
    _red "Error: This script only supports macOS"
    _yellow "Detected OS: $OSTYPE"
    return 1
  fi
  return 0
}

# Get macOS shell profile file
_get_shell_profile() {
...
# Install AWS CLI (macOS only)
_install_aws_cli() {
  if ! _check_macos; then
    return 1
  fi
  
  local install_success=false
  _blue "Installing AWS CLI on macOS using official method..."
  # Reference: https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html
  
  # Try Homebrew first (preferred method for macOS)
  if command -v brew &> /dev/null; then
    _blue "Installing AWS CLI using Homebrew..."
    if brew install awscli 2>&1; then
      install_success=true
      _green "AWS CLI installed successfully using Homebrew"
    fi
  fi
  
  # If Homebrew failed or not available, use official .pkg installer
  if [[ "$install_success" == "false" ]]; then
    _blue "Installing AWS CLI using official .pkg installer..."
    local pkg_file="/tmp/AWSCLIV2.pkg"
    
    # Download AWS CLI v2 for macOS (universal package works for both ARM64 and x86_64)
    _blue "Downloading AWS CLI v2 for macOS..."
    if curl -sSL "https://awscli.amazonaws.com/AWSCLIV2.pkg" -o "$pkg_file"; then
      _blue "Installing AWS CLI using official installer (this may require sudo password)..."
      local install_output
      install_output=$(sudo installer -pkg "$pkg_file" -target / 2>&1)
      local install_exit_code=$?
      if [[ $install_exit_code -eq 0 ]]; then
        install_success=true
        _green "AWS CLI installed successfully using official method"
        _blue "Installation location: /usr/local/aws-cli"
        _blue "Symlink created at: /usr/local/bin/aws"
      else
      ...
      _yellow "Failed to download AWS CLI package"
    fi
  fi
  ...
  if [[ "$install_success" == "true" ]]; then
    ...
    elif [[ -f "/usr/local/aws-cli/aws" ]]; then
      aws_cmd="/usr/local/aws-cli/aws"
...
_install_kubectl() {
  ...
  local install_success=false
  _blue "Installing kubectl on macOS..."
  
  # Try Homebrew first (preferred method for macOS)
  if command -v brew &> /dev/null; then
    _blue "Installing kubectl using Homebrew..."
    if brew install kubectl 2>&1; then
      install_success=true
    fi
  fi
  
  # If Homebrew failed or not available, use official method
  if [[ "$install_success" == "false" ]]; then
    local arch=$(uname -m)
    case "$arch" in
      x86_64) arch="amd64" ;;
    ...
    local url="https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/darwin/${arch}/kubectl"
...
# Install krew (kubectl plugin manager) - macOS only
_install_krew() {
  if ! _check_macos; then
    return 1
  fi
  ...
  _blue "Installing krew on macOS..."
...
_install_helm() {
  ...
  local install_success=false
  _blue "Installing Helm on macOS..."
  
  # Try Homebrew first (preferred method for macOS)
  if command -v brew &> /dev/null; then
    _blue "Installing Helm using Homebrew..."
    if brew install helm 2>&1; then
      install_success=true
    fi
  fi
  
  # If Homebrew failed or not available, use official script
  if [[ "$install_success" == "false" ]]; then
...
_install_kustomize() {
  ...
  local install_success=false
  _blue "Installing Kustomize on macOS..."
  
  # Try Homebrew first (preferred method for macOS)
  if command -v brew &> /dev/null; then
    _blue "Installing Kustomize using Homebrew..."
    if brew install kustomize 2>&1; then
      install_success=true
    fi
  fi
...
# Install tfenv (Terraform version manager) - macOS only
_install_tfenv() {
  if ! _check_macos; then
    return 1
  fi
  ...
  local install_success=false
  _blue "Installing tfenv on macOS..."
  
  # Check for git (required for tfenv)
  if ! command -v git &> /dev/null; then
    _blue "git not found, installing via Homebrew..."
    if command -v brew &> /dev/null; then
      brew install git 2>&1 || return 1
    else
      _yellow "git is required but Homebrew is not available"
      _yellow "Please install git manually or install Homebrew: https://brew.sh"
      return 1
    fi
  fi
  ...
  if [[ "$install_success" == "true" ]] || [[ -d "$tfenv_dir" ]]; then
    ...
    if command -v tfenv &> /dev/null; then
      ...
      local shell_profile=$(_get_shell_profile)
      _yellow "Add to your shell profile ($shell_profile): export PATH=\"\${TFENV_ROOT:-\$HOME/.tfenv}/bin:\$PATH\""
...
_install_packer() {
  ...
  local install_success=false
  _blue "Installing Packer on macOS..."
  
  # Try Homebrew first (preferred method for macOS)
  if command -v brew &> /dev/null; then
    _blue "Installing Packer using Homebrew..."
    if brew install packer 2>&1; then
      install_success=true
    fi
  fi
...
_setup_kubectl_alias() {
  ...
  # Create alias
  if ! grep -q "alias k=kubectl" "$shell_profile" 2>/dev/null; then
    echo "" >> "$shell_profile"
    echo "# kubectl alias" >> "$shell_profile"
    echo "alias k=kubectl" >> "$shell_profile"
  fi
  
  # Setup completion for kubectl
  if ! grep -q "kubectl completion" "$shell_profile" 2>/dev/null; then
    echo "" >> "$shell_profile"
    echo "# kubectl completion" >> "$shell_profile"
    echo "$completion_cmd" >> "$shell_profile"
    if [[ -z "${ZSH_VERSION:-}" ]]; then
      echo "complete -F __start_kubectl k" >> "$shell_profile"
    fi
  fi
  
  # Always setup for current session (even if already in shell profile)
  ...
  if alias k &> /dev/null; then
    ...
    _yellow "Note: Alias is now available in this session. For future sessions, restart your terminal or run: source $shell_profile"
  else
    _yellow "⚠ kubectl alias 'k' configured in $shell_profile but not available in current session"
    _yellow "Run: source $shell_profile or restart your terminal"
  fi
  return 0
}

# Install fzf (fuzzy finder) - macOS only
_install_fzf() {
  if ! _check_macos; then
    return 1
  fi
  ...
  local install_success=false
  _blue "Installing fzf on macOS..."
  
  # Try Homebrew first (preferred method for macOS)
  if command -v brew &> /dev/null; then
    _blue "Installing fzf using Homebrew..."
    if brew install fzf 2>&1; then
      install_success=true
      # Homebrew installs fzf and sets up key bindings
      _green "✓ fzf installed successfully using Homebrew"
      return 0
    fi
  fi
  
  # If Homebrew failed or not available, use git clone method
  if [[ "$install_success" == "false" ]]; then
    ...
    if [[ "$install_success" == "true" ]] || [[ -d "$fzf_dir" ]]; then
      ...
      if bash "$fzf_dir/install" --bin 2>&1; then
        ...
        local shell_profile=$(_get_shell_profile)
        local fzf_source_file=""
        
        # Determine fzf source file based on shell
        if [[ -n "${ZSH_VERSION:-}" ]]; then
          fzf_source_file="~/.fzf.zsh"
        else
          fzf_source_file="~/.fzf.bash"
        fi
        
        # Setup fzf key bindings and completion
        if ! grep -q "source.*fzf" "$shell_profile" 2>/dev/null; then
          echo "" >> "$shell_profile"
          echo "# fzf key bindings and completion" >> "$shell_profile"
          echo "[ -f $fzf_source_file ] && source $fzf_source_file" >> "$shell_profile"
        fi
        ...
        if command -v fzf &> /dev/null; then
          ...
          _yellow "Add to your shell profile ($shell_profile): export PATH=\"\${FZF_ROOT:-\$HOME/.fzf}/bin:\$PATH\""
...
_setup_fzf_config() {
  ...
  if ! grep -q "# fzf aliases and functions" "$shell_profile" 2>/dev/null; then
    echo "" >> "$shell_profile"
    echo "# fzf aliases and functions" >> "$shell_profile"
    echo "export FZF_DEFAULT_OPTS='--height 40% --layout=reverse --border'" >> "$shell_profile"
    echo "export FZF_CTRL_T_OPTS=\"--preview 'bat --color=always --style=header,grid --line-range :300 {}'\"" >> "$shell_profile"
    echo "alias fzfv='fzf --preview \"bat --color=always --style=header,grid --line-range :300 {}\"'" >> "$shell_profile"
    echo "alias fzfg='fzf --preview \"git diff {}\"'" >> "$shell_profile"
  fi
...
_check_kubectl_alias() {
  if command -v kubectl &> /dev/null; then
    ...
    if [[ "$auto_setup" == "true" ]]; then
      local shell_profile=$(_get_shell_profile)
      # Always ensure alias is set up in shell profile for future sessions
      if ! grep -q "alias k=kubectl" "$shell_profile" 2>/dev/null; then
        _setup_kubectl_alias || true
      else
        # Even if alias exists in shell profile, ensure it's set in current session
        alias k=kubectl 2>/dev/null || true
        if command -v kubectl &> /dev/null; then
          # Load kubectl completion for current session if not already loaded
          if [[ -n "${ZSH_VERSION:-}" ]]; then
        ...
        if alias k &> /dev/null; then
          ...
          _yellow "⚠ kubectl alias 'k' found in $shell_profile but failed to set in current session"
          _yellow "Run: source $shell_profile or restart your terminal"
        fi
      fi
    fi
...
# Check macOS before proceeding
# Only check if script is being sourced, not executed directly
# This prevents errors when shell profile is executed directly
if [[ "${BASH_SOURCE[0]}" != "${0}" ]]; then
  if ! _check_macos; then
    _red "This script only supports macOS. Exiting."
...
if [[ "${BASH_SOURCE[0]}" != "${0}" ]]; then
  ...
  _green "✓ AWS utilities loaded for macOS. Type 'aws-help' for available commands."
fi
